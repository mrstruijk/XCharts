...
namespace XCharts.Runtime
{
    ...
    internal class BaseScatterHandler<T> : SerieHandler<T> where T : BaseScatter
    {
        ...
        protected virtual void DrawScatterSerie(VertexHelper vh, BaseScatter serie)
        {
            ...
            var maxCount = serie.maxShow > 0 ? serie.maxShow > serie.dataCount ? serie.dataCount : serie.maxShow : serie.dataCount;
            ...
            foreach (var serieData in dataList)
            {
                ...
                var xValue = serieData.GetCurrData(0, 0, isFadeOut ? 0 : dataChangeDuration, unscaledTime, xAxis.inverse);
                var yValue = serieData.GetCurrData(1, 0, isFadeOut ? 0 : dataChangeDuration, unscaledTime, yAxis.inverse);

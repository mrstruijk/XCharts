    ...
    internal sealed class PieHandler : SerieHandler<Pie>
    {
        ...
        public override void OnPointerDown(PointerEventData eventData)
        {
            ...
            if (dataIndex >= 0)
            {
                ...
                for (var j = 0; j < serie.data.Count; j++)
                {
                    if (j == dataIndex)
                    {
                        serie.data[j].context.selected = !serie.data[j].context.selected;
                    }
                    else
                    {
                        serie.data[j].context.selected = false;
                    }
                }
            }
        ...
        private void UpdateRuntimeData(Serie serie)
        {
            ...
            for (var n = 0; n < data.Count; n++)
            {
                ...
                var degree = serie.pieRoseType == RoseType.Area ? totalDegree / showdataCount : (float) (totalDegree * value / dataTotalFilterMinAngle);
        ...
        private void UpdateSerieDataRadius(SerieData serieData, double value)
        {
            ...
            else
            {
                ...
                serieData.context.outsideRadius = serie.pieRoseType > 0 ? minInsideRadius + (float) ((serie.context.outsideRadius - minInsideRadius) * value / serie.context.dataMax) : serie.context.outsideRadius;
            }
        ...
        private void DrawPie(VertexHelper vh, Pie serie)
        {
            ...
            if (data.Count == 0)
            {
                ...
                var fillColor = ChartHelper.IsClearColor(itemStyle.backgroundColor) ? (Color32) chart.theme.legend.unableColor : itemStyle.backgroundColor;
            ...
            foreach (var serieData in serie.data)
            {
                ...
                if (SerieLabelHelper.CanShowLabel(serie, serieData, serieLabel, 1))
                {
                        ...
                        labelLine != null && labelLine.show)
                    {
                        ...
                        {
                            if (isTop && !labelLine.startSymbol.show)
                            {
                                continue;
                            }
                        }

                        var color = ChartHelper.IsClearColor(labelLine.lineColor) ? chart.theme.GetColor(colorIndex) : labelLine.lineColor;
        ...
        private int GetPiePosIndex(Serie serie, Vector2 local)
        {
            ...
            for (var i = 0; i < serie.data.Count; i++)
            {
                ...
                {
                    var ndist = serieData.selected || serieData.context.selected ? Vector2.Distance(local, serieData.context.offsetCenter) : dist;

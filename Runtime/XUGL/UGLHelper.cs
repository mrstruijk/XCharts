...
namespace XUGL
{
    public static class UGLHelper
    {
                                                 ...
                                                 float smoothness = 2f, float k = 2.0f)
        {
            var dir = (ep - sp).normalized;
            var dist = Vector3.Distance(sp, ep);
            var cp1 = sp + dist / k * dir * 1;
            var cp2 = sp + dist / k * dir * (k - 1);
            cp1.x = sp.x;
            cp2.x = ep.x;
            var segment = (int) (dist / (smoothness <= 0 ? 2f : smoothness));
            GetBezierList2(ref posList, sp, ep, segment, cp1, cp2);
                                         ...
                                         ref bool bitp, ref bool bibp, int debugIndex = 0)
        {
            ...
            if (!GetIntersection(ltp, ltp + ldist, ntp, ntp + rdist, ref itp))
            {
                itp = cp - dir1v;
                clp = cp - dir1v;
                crp = cp - dir2v;
            ...
            if (!GetIntersection(lbp, lbp + ldist, nbp, nbp + rdist, ref ibp))
            {
                ibp = cp + dir1v;
                clp = cp + dir1v;
                crp = cp + dir2v;
        ...
        public static bool IsPointInPolygon(Vector3 p, List<Vector3> polyons)
        {
            ...
            for (var i = 0; i < polyons.Count; j = i++)
            {
                var pi = polyons[i];
                var pj = polyons[j];
        ...
        public static bool IsPointInPolygon(Vector3 p, params Vector3[] polyons)
        {
            ...
            for (var i = 0; i < polyons.Length; j = i++)
            {
                var pi = polyons[i];
                var pj = polyons[j];

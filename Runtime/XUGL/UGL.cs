...
namespace XUGL
{
    ...
    public static class UGL
    {
        ...
        private static readonly Color32 s_ClearColor32 = new(0, 0, 0, 0);
        private static readonly Vector2 s_ZeroVector2 = Vector2.zero;
        private static readonly UIVertex[] s_Vertex = new UIVertex[4];
        private static List<Vector3> s_CurvesPosList = new();
        ...
        public static void DrawLine(VertexHelper vh, Vector3 startPoint, Vector3 endPoint, float width, Color32 color, Color32 toColor)
        {
            ...
            var v = Vector3.Cross(endPoint - startPoint, Vector3.forward).normalized * width;
                                           ...
                                           List<Vector3> posList = null)
        {
            ...
            var segment = Mathf.CeilToInt(dist / (dashLength + 2 * gapLength + dotLength));
            var dir = (endPoint - startPoint).normalized;
            Vector3 sp = startPoint, np;
                                              ...
                                              List<Vector3> posList = null)
        {
            ...
            var segment = Mathf.CeilToInt(dist / (dashLength + 3 * gapLength + 2 * dotLength));
            var dir = (endPoint - startPoint).normalized;
            Vector3 sp = startPoint, np;
        ...
        {
            DrawRectangle(vh, rect.center, rect.width / 2, rect.height / 2, color, color);
        }


        public static void DrawRectangle(VertexHelper vh, Rect rect, Color32 color, Color32 toColor)
        {
            DrawRectangle(vh, rect.center, rect.width / 2, rect.height / 2, color, toColor);
        }
        ...
        {
            DrawRectangle(vh, rect.center, rect.width / 2 - border, rect.height / 2 - border, color, toColor);
        }
                                              ...
                                              float smoothness = 2, bool invert = false)
        {
            ...
            if (needRound)
            {
                ...
                if (horizontal)
                {
                    ...
                    if (roundLbRight.x > roundRb.x)
                    {
                        roundLbRight.x = roundRb.x;
                    }

                    if (lbIn2Right.x > roundRb.x)
                    {
                        lbIn2Right.x = roundRb.x;
                    }

                    var ltIn2Right = ltIn2 + (maxLeft - brLt) * Vector3.right;
                    var roundLtRight = roundLt + (maxLeft - brLt) * Vector3.right;

                    if (ltIn2Right.x > roundRt.x)
                    {
                        ltIn2Right.x = roundRt.x;
                    }

                    if (roundLtRight.x > roundRt.x)
                    {
                        roundLtRight.x = roundRt.x;
                    }

                    var roundRtLeft = roundRt + (maxRight - brRt) * Vector3.left;
                    var rtInLeft = rtIn + (maxRight - brRt) * Vector3.left;

                    if (roundRtLeft.x < roundLt.x)
                    {
                        roundRtLeft.x = roundLt.x;
                    }

                    if (rtInLeft.x < roundLt.x)
                    {
                        rtInLeft.x = roundLt.x;
                    }

                    var rbIn2Left = rbIn2 + (maxRight - brRb) * Vector3.left;
                    var roundRbLeft = roundRb + (maxRight - brRb) * Vector3.left;

                    if (rbIn2Left.x < roundLb.x)
                    {
                        rbIn2Left.x = roundLb.x;
                    }

                    if (roundRbLeft.x < roundLb.x)
                    {
                        roundRbLeft.x = roundLb.x;
                    }
                ...
                else
                {
                    ...
                    if (roundLtDown.y < roundLb.y)
                    {
                        roundLtDown.y = roundLb.y;
                    }

                    if (ltInDown.y < roundLb.y)
                    {
                        ltInDown.y = roundLb.y;
                    }

                    var rtIn2Down = rtIn2 + (maxup - brRt) * Vector3.down;
                    var roundRtDown = roundRt + (maxup - brRt) * Vector3.down;

                    if (rtIn2Down.y < roundRb.y)
                    {
                        rtIn2Down.y = roundRb.y;
                    }

                    if (roundRtDown.y < roundRb.y)
                    {
                        roundRtDown.y = roundRb.y;
                    }

                    var lbInUp = lbIn + (maxdown - brLb) * Vector3.up;
                    var roundLbUp = roundLb + (maxdown - brLb) * Vector3.up;

                    if (lbInUp.y > roundLt.y)
                    {
                        lbInUp.y = roundLt.y;
                    }

                    if (roundLbUp.y > roundLt.y)
                    {
                        roundLbUp.y = roundLt.y;
                    }

                    var roundRbUp = roundRb + (maxdown - brRb) * Vector3.up;
                    var rbInUp = rbIn + (maxdown - brRb) * Vector3.up;

                    if (roundRbUp.y > roundRt.y)
                    {
                        roundRbUp.y = roundRt.y;
                    }

                    if (rbInUp.y > roundRt.y)
                    {
                        rbInUp.y = roundRt.y;
                    }
                                                        ...
                                                        float rotate = 0, float smoothness = 2)
        {
            ...
            if (borderWidth > 0)
            {
                DrawBorder(vh, rect, borderWidth, borderColor, rotate, cornerRadius, true, smoothness);
            }
        }
        ...
        public static void DrawBorder(VertexHelper vh, Vector3 center, float rectWidth, float rectHeight,
                                      ...
                                      bool horizontal = false, float smoothness = 1f, bool invertCorner = false, float extWidth = 0)
        {
            if (borderWidth == 0 || UGLHelper.IsClearColor(color))
            {
                return;
            }
                                      ...
                                      float smoothness, int gradientType = 0, bool isYAxis = false)
        {
            ...
            var segments = (int) (2 * Mathf.PI * radius * (Mathf.Abs(toDegree - startDegree) / 360) / smoothness);
            ...
            var startAngle = startDegree * Mathf.Deg2Rad;
            var toAngle = toDegree * Mathf.Deg2Rad;
            var realStartAngle = startAngle;
            var realToAngle = toAngle;
            var halfAngle = (toAngle - startAngle) / 2;
            float borderAngle = 0;
            float spaceAngle = 0;
            ...
            var segmentAngle = (realToAngle - realStartAngle) / segments;
            var isLeft = startDegree >= 180;

            for (var i = 0; i <= segments; i++)
            {
                var currAngle = realStartAngle + i * segmentAngle;
        ...
        {
            if (toDegree - startDegree == 0)
            {
                return;
            }

            if (gap > 0 && Mathf.Abs(toDegree - startDegree) >= 360)
            {
                gap = 0;
            }
            ...
            var segments = (int) (2 * Mathf.PI * outsideRadius * (diffAngle * Mathf.Rad2Deg / 360) / smoothness);
            ...
            var realStartOutAngle = startAngle;
            var realToOutAngle = toAngle;
            var realStartInAngle = startAngle;
            var realToInAngle = toAngle;
            var halfAngle = (toAngle - startAngle) / 2;
            ...
            if (needBorder || needSpace)
            {
                if (needSpace)
                {
                    ...
                    if (clockwise)
                    {
                        p1 = UGLHelper.GetPos(center, insideRadius, startAngle + spaceInAngle);
                        e1 = UGLHelper.GetPos(center, insideRadius, toAngle - spaceInAngle);
                ...
                if (needBorder && !isCircle)
                {
                    ...
                    if (clockwise)
                    {
                        ...
                        var newp1 = UGLHelper.GetPos(center, insideRadius, startAngle + spaceInAngle + borderInAngle);
                        var newp2 = UGLHelper.GetPos(center, outsideRadius, realStartOutAngle);

                        if (!roundCap)
                        {
                            DrawQuadrilateral(vh, newp2, newp1, p1, p2, borderColor);
                        }
                        ...
                        {
                            var newe1 = UGLHelper.GetPos(center, insideRadius, toAngle - spaceInAngle - borderInAngle);
                            var newe2 = UGLHelper.GetPos(center, outsideRadius, realToOutAngle);

                            if (!roundCap)
                            {
                                DrawQuadrilateral(vh, newe2, e2, e1, newe1, borderColor);
                            }
            ...
            if (roundCap)
            {
                ...
                if (clockwise)
                {
                    ...
                    if (realToOutAngle < realStartOutAngle)
                    {
                        realToOutAngle = realStartOutAngle;
                    }
                }
                else
                {
            ...
            for (var i = 0; i <= segments; i++)
            {
                var currAngle = realStartInAngle + i * segmentAngle;
                ...
                if (isGradient)
                {
                    if (radiusGradient)
                    {
                        ...
                        {
                            DrawTriangle(vh, p1, p2, p3, color, toColor, toColor);
                        }
                    ...
                    else
                    {
                        ...
                        if (i == 0 && (needSpace || needBorder))
                        {
                            DrawTriangle(vh, p1, p2, p3, color, tcolor, tcolor);
                        }
                ...
                else
                {
                    if (i == 0 && (needSpace || needBorder))
                    {
                        DrawTriangle(vh, p1, p2, p3, color);
                    }
            ...
            if (needBorder || needSpace || roundCap)
            {
                if (clockwise)
                {
                    ...
                    if (isInAngleFixed)
                    {
                        DrawQuadrilateral(vh, p2, e2, e1, p1, color, toColor);
                    }
                    else
                    {
                        DrawTriangle(vh, p2, e2, p1, color, color, toColor);
                    }
                    ...
                    if (needBorder)
                    {
                        ...
                        if (realToDegree < realStartOutAngle)
                        {
                            realToDegree = realStartOutAngle;
                        }
                        ...
                        if (inToDegree < inStartDegree)
                        {
                            inToDegree = inStartDegree;
                        }
                                              ...
                                              Color32 lineColor, Direction dire, float currProgress = float.NaN)
        {
            if (curvesPosList.Count > 1)
            {
                ...
                for (var i = 1; i < curvesPosList.Count; i++)
                {
                    to = curvesPosList[i];
        ...
        {
            if (topList.Count < 2 || bottomList.Count < 2)
            {
                return;
            }
        ...
        public static void DrawPlus(VertexHelper vh, Vector3 center, float radius, float tickness, Color32 color)
        {
            ...
            DrawLine(vh, xPos1, xPos2, tickness, color);
            DrawLine(vh, yPos1, yPos2, tickness, color);
        }

...
namespace XCharts.Runtime
{
    ...
    public partial class BaseChart : BaseGraph, ISerializationCallbackReceiver
    {
        ...
        [SerializeField] protected ThemeStyle m_Theme = new();
        [SerializeField] protected Settings m_Settings;
        [SerializeField] protected DebugInfo m_DebugInfo = new();
        [SerializeField] protected bool m_ChartInited = false;
        protected List<Serie> m_Series = new();
        protected List<MainComponent> m_Components = new();

        protected Dictionary<Type, FieldInfo> m_TypeListForComponent = new();
        protected Dictionary<Type, FieldInfo> m_TypeListForSerie = new();

        protected Dictionary<Type, List<MainComponent>> m_ComponentMaps = new();
        ...
        protected Rect m_ChartRect = new(0, 0, 0, 0);
        ...
        internal protected List<string> m_LegendRealShowName = new();
        protected List<Painter> m_PainterList = new();
        ...
        protected List<MainComponentHandler> m_ComponentHandlers = new();
        protected List<SerieHandler> m_SerieHandlers = new();
        ...
        public override HideFlags chartHideFlags => m_DebugInfo.showAllChartObject ? HideFlags.None : HideFlags.HideInHierarchy;
        ...
        protected override void InitPainter()
        {
            ...
            for (var i = 0; i < settings.maxPainter; i++)
            {
        ...
        internal virtual void OnSerieDataUpdate(int serieIndex)
        {
            foreach (var handler in m_ComponentHandlers)
            {
                handler.OnSerieDataUpdate(serieIndex);
            }
        }
        ...
        protected override void OnLocalPositionChanged()
        {
            ...
            if (TryGetChartComponent(out background))
            {
        ...
        protected virtual void OnDrawPainterSerie(VertexHelper vh, Painter painter)
        {
            ...
            for (var i = painter.index * rate; i < (painter.index + 1) * rate && i < maxSeries; i++)
            {
                var serie = m_Series[i];
        ...
        protected virtual void OnDrawPainterUpper(VertexHelper vh, Painter painter)
        {
            ...
            foreach (var draw in m_ComponentHandlers)
            {
                draw.DrawUpper(vh);
            }
        ...
        protected virtual void DrawBackground(VertexHelper vh)
        {
            ...
            var p1 = new Vector3(chartX, chartY + chartHeight);
            var p2 = new Vector3(chartX + chartWidth, chartY + chartHeight);
            var p3 = new Vector3(chartX + chartWidth, chartY);
            var p4 = new Vector3(chartX, chartY);
        ...
        [SerializeField] [ListForComponent(typeof(AngleAxis))] private List<AngleAxis> m_AngleAxes = new();
        [SerializeField] [ListForComponent(typeof(Background))] private List<Background> m_Backgrounds = new();
        [SerializeField] [ListForComponent(typeof(DataZoom))] private List<DataZoom> m_DataZooms = new();
        [SerializeField] [ListForComponent(typeof(GridCoord))] private List<GridCoord> m_Grids = new();
        [SerializeField] [ListForComponent(typeof(GridLayout))] private List<GridLayout> m_GridsLayout = new();

        [SerializeField] [ListForComponent(typeof(Legend))] private List<Legend> m_Legends = new();
        [SerializeField] [ListForComponent(typeof(MarkLine))] private List<MarkLine> m_MarkLines = new();
        [SerializeField] [ListForComponent(typeof(MarkArea))] private List<MarkArea> m_MarkAreas = new();
        [SerializeField] [ListForComponent(typeof(PolarCoord))] private List<PolarCoord> m_Polars = new();
        [SerializeField] [ListForComponent(typeof(RadarCoord))] private List<RadarCoord> m_Radars = new();
        [SerializeField] [ListForComponent(typeof(RadiusAxis))] private List<RadiusAxis> m_RadiusAxes = new();
        [SerializeField] [ListForComponent(typeof(Title))] private List<Title> m_Titles = new();
        [SerializeField] [ListForComponent(typeof(Tooltip))] private List<Tooltip> m_Tooltips = new();
        [SerializeField] [ListForComponent(typeof(VisualMap))] private List<VisualMap> m_VisualMaps = new();
        [SerializeField] [ListForComponent(typeof(XAxis))] private List<XAxis> m_XAxes = new();
        [SerializeField] [ListForComponent(typeof(YAxis))] private List<YAxis> m_YAxes = new();
        [SerializeField] [ListForComponent(typeof(SingleAxis))] private List<SingleAxis> m_SingleAxes = new();
        [SerializeField] [ListForComponent(typeof(ParallelCoord))] private List<ParallelCoord> m_Parallels = new();
        [SerializeField] [ListForComponent(typeof(ParallelAxis))] private List<ParallelAxis> m_ParallelAxes = new();
        [SerializeField] [ListForComponent(typeof(Comment))] private List<Comment> m_Comments = new();

        [SerializeField] [ListForSerie(typeof(Bar))] private List<Bar> m_SerieBars = new();
        [SerializeField] [ListForSerie(typeof(Candlestick))] private List<Candlestick> m_SerieCandlesticks = new();
        [SerializeField] [ListForSerie(typeof(EffectScatter))] private List<EffectScatter> m_SerieEffectScatters = new();
        [SerializeField] [ListForSerie(typeof(Heatmap))] private List<Heatmap> m_SerieHeatmaps = new();
        [SerializeField] [ListForSerie(typeof(Line))] private List<Line> m_SerieLines = new();
        [SerializeField] [ListForSerie(typeof(Pie))] private List<Pie> m_SeriePies = new();
        [SerializeField] [ListForSerie(typeof(Radar))] private List<Radar> m_SerieRadars = new();
        [SerializeField] [ListForSerie(typeof(Ring))] private List<Ring> m_SerieRings = new();
        [SerializeField] [ListForSerie(typeof(Scatter))] private List<Scatter> m_SerieScatters = new();
        [SerializeField] [ListForSerie(typeof(Parallel))] private List<Parallel> m_SerieParallels = new();
        [SerializeField] [ListForSerie(typeof(SimplifiedLine))] private List<SimplifiedLine> m_SerieSimplifiedLines = new();
        [SerializeField] [ListForSerie(typeof(SimplifiedBar))] private List<SimplifiedBar> m_SerieSimplifiedBars = new();
        [SerializeField] [ListForSerie(typeof(SimplifiedCandlestick))] private List<SimplifiedCandlestick> m_SerieSimplifiedCandlesticks = new();

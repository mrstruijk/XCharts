...
namespace XCharts.Runtime
{
    ...
    public class LabelStyle : ChildComponent, ISerieComponent, ISerieDataComponent
    {
        ...
        [SerializeField] private Position m_Position = Position.Default;
        ...
        [SerializeField] protected IconStyle m_Icon = new();
        [SerializeField] protected ImageStyle m_Background = new();
        [SerializeField] protected TextPadding m_TextPadding = new();
        [SerializeField] protected TextStyle m_TextStyle = new();
        ...
        /// <summary>
        ///     label content string template formatter. \n line wrapping is supported. Formatters for some components will not
        ///     take effect. <br />
        ...
        /// <summary>
        ///     Whether to automatically offset. When turned on, the Y offset will automatically determine the opening of the curve
        ///     to determine whether to offset up or down.
        ...
        public virtual string GetFormatterContent(int labelIndex, double value, double minValue, double maxValue, bool isLog = false)
        {
            ...
            if (string.IsNullOrEmpty(m_Formatter))
            {
                ...
                {
                    var minAcc = MathUtil.GetPrecision(minValue);
                    var maxAcc = MathUtil.GetPrecision(maxValue);
                    var curAcc = MathUtil.GetPrecision(value);
                    var acc = Mathf.Max(Mathf.Max(minAcc, maxAcc), curAcc);

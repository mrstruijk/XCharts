...
namespace XCharts.Runtime
{
    ...
    public class Settings : MainComponent
    {
        ...
        [SerializeField] private float[] m_LegendIconCornerRadius = {0.25f, 0.25f, 0.25f, 0.25f};
        ...
        {
            get => m_LineSmoothStyle;
            set
            {
                if (PropertyUtil.SetStruct(ref m_LineSmoothStyle, value < 0 ? 1f : value))
                {
                    SetVerticesDirty();
                }
            }
        ...
        {
            get => m_LineSmoothness;
            set
            {
                if (PropertyUtil.SetStruct(ref m_LineSmoothness, value < 0 ? 1f : value))
                {
                    SetVerticesDirty();
                }
            }
        ...
        ///     the higher the number of vertices. When the area with gradient is filled, the larger the value, the worse the
        ///     transition effect.
        ///     ||线段的分割距离。普通折线图的线是由很多线段组成，段数由该数值决定。值越小段数越多，但顶点数也会随之增加。当开启有渐变的区域填充时，数值越大渐变过渡效果越差。
        /// </summary>
        public float lineSegmentDistance
        {
            get => m_LineSegmentDistance;
            set
            {
                if (PropertyUtil.SetStruct(ref m_LineSegmentDistance, value < 0 ? 1f : value))
                {
                    SetVerticesDirty();
                }
            }
        ...
        {
            get => m_CicleSmoothness;
            set
            {
                if (PropertyUtil.SetStruct(ref m_CicleSmoothness, value < 0 ? 1f : value))
                {
                    SetVerticesDirty();
                }
            }
        ...
        {
            get => m_LegendIconCornerRadius;
            set
            {
                if (PropertyUtil.SetClass(ref m_LegendIconCornerRadius, value, true))
                {
                    SetVerticesDirty();
                }
            }
        ...
        public static Settings DefaultSettings
        {
            get
            {
                return new Settings
                {
                    ...
                    m_LegendIconCornerRadius = new[] {0.25f, 0.25f, 0.25f, 0.25f}
                };
            }
        }

...
namespace XCharts.Runtime
{
    ...
    internal sealed class MarkLineHandler : MainComponentHandler<MarkLine>
    {
        ...
        private readonly Dictionary<int, List<MarkLineData>> m_TempGroupData = new();
        ...
        private void ResetTempMarkLineGroupData(MarkLine markLine)
        {
            ...
            for (var i = 0; i < markLine.data.Count; i++)
            {
                                     ...
                                     int serieDataCount)
        {
            switch (data.type)
            {
                ...
                case MarkLineType.None:
                    ...
                    pX = data.xPosition != 0 ? grid.context.x + data.xPosition : GetAxisPosition(grid, xAxis, dataZoom, serieDataCount, data.xValue);
                    pY = data.yPosition != 0 ? grid.context.y + data.yPosition : GetAxisPosition(grid, yAxis, dataZoom, serieDataCount, data.yValue);
                    data.runtimeValue = data.yValue;

...
namespace XCharts.Runtime
{
    ...
    internal sealed class DataZoomHandler : MainComponentHandler<DataZoom>
    {
        ...
        {
            var grid = chart.GetGridOfDataZoom(dataZoom);
            var deltaPercent = dataZoom.orient == Orient.Horizonal ? Mathf.Abs(delta / grid.context.width * 100) : Mathf.Abs(delta / grid.context.height * 100);
        ...
        private void DrawHorizonalDataZoomSlider(VertexHelper vh, DataZoom dataZoom)
        {
            ...
            if (dataZoom.showDataShadow && chart.series.Count > 0)
            {
                ...
                Axis axis = chart.GetChartComponent<YAxis>();
                var showData = serie.GetDataList();
                var scaleWid = dataZoom.context.width / (showData.Count - 1);
                var lp = Vector3.zero;
                var np = Vector3.zero;
                double minValue = 0;
                double maxValue = 0;
                ...
                var totalAverage = serie.sampleAverage > 0 ? serie.sampleAverage : DataHelper.DataAverage(ref showData, serie.sampleType, serie.minShow, maxCount, rate);
                var dataChanging = false;
                var animationDuration = serie.animation.GetChangeDuration();
                var dataAddDuration = serie.animation.GetAdditionDuration();
                var unscaledTime = serie.animation.unscaledTime;

                for (var i = 0; i < maxCount; i += rate)
                {
                    var value = DataHelper.SampleValue(ref showData, serie.sampleType, rate, serie.minShow, maxCount, totalAverage, i,
                        dataAddDuration, animationDuration, ref dataChanging, axis, unscaledTime);

                    var pX = dataZoom.context.x + i * scaleWid;
                    var dataHig = (float) (maxValue - minValue == 0 ? 0 : (value - minValue) / (maxValue - minValue) * dataZoom.context.height);
                    np = new Vector3(pX, chart.chartY + dataZoom.bottom + dataHig);

                    if (i > 0)
                    {
                        UGL.DrawLine(vh, lp, np, lineWidth, lineColor);
                        var alp = new Vector3(lp.x, lp.y - lineWidth);
                        var anp = new Vector3(np.x, np.y - lineWidth);

                        var tnp = new Vector3(np.x, chart.chartY + dataZoom.bottom + lineWidth);
                        var tlp = new Vector3(lp.x, chart.chartY + dataZoom.bottom + lineWidth);
        ...
        private void DrawVerticalDataZoomSlider(VertexHelper vh, DataZoom dataZoom)
        {
            ...
            if (dataZoom.showDataShadow && chart.series.Count > 0)
            {
                ...
                Axis axis = chart.GetChartComponent<YAxis>();
                var showData = serie.GetDataList();
                var scaleWid = dataZoom.context.height / (showData.Count - 1);
                var lp = Vector3.zero;
                var np = Vector3.zero;
                double minValue = 0;
                double maxValue = 0;
                ...
                for (var i = 0; i < maxCount; i += rate)
                {
                    ...
                    var pY = dataZoom.context.y + i * scaleWid;
                    var dataHig = maxValue - minValue == 0 ? 0 : (float) ((value - minValue) / (maxValue - minValue) * dataZoom.context.width);

...
namespace XCharts.Runtime
{
    ...
    public class Axis : MainComponent
    {
        ...
        [SerializeField] protected List<Sprite> m_Icons = new();
        [SerializeField] protected List<string> m_Data = new();
        ...
        [SerializeField] protected AxisAnimation m_Animation = new();
        ...
        [SerializeField] [Since("v3.4.0")] protected LabelStyle m_IndicatorLabel = new() {numericFormatter = "f2"};

        public AxisContext context = new();
        ...
        {
            get => IsCategory() ? m_BoundaryGap : false;
            set
            {
        ...
        public double ceilRate
        {
            ...
            set
            {
                if (PropertyUtil.SetStruct(ref m_CeilRate, value < 0 ? 0 : value))
                {
                    SetAllDirty();
                }
            }
        ...
        {
            get => m_MinCategorySpacing;
            set
            {
                if (PropertyUtil.SetStruct(ref m_MinCategorySpacing, value))
                {
                    SetAllDirty();
                }
            }
        ...
        public float GetDistance(double value, float axisLength = 0)
        {
            ...
            if (IsLog())
            {
                ...
                return axisLength * ((logValue - logMin) / (logMax - logMin));
            }
        ...
        {
            for (var i = 0; i < context.labelObjectList.Count; i++)
            {
        ...
        internal void UpdateMinMaxValue(double minValue, double maxValue, bool needAnimation = false)
        {
            ...
            if (context.minMaxRange != tempRange)
            {
                ...
                if (type == AxisType.Value && interval > 0)
                {
                    SetComponentDirty();
                }
            }
        ...
        public double GetLogMinIndex()
        {
            ...
            return logBaseE ? Math.Log(context.minValue) : Math.Log(context.minValue, logBase);
        }
        ...
        public double GetLogMaxIndex()
        {
            ...
            return logBaseE ? Math.Log(context.maxValue) : Math.Log(context.maxValue, logBase);
        }
        ...
        {
            context.offset = context.minValue > 0 || context.minMaxRange == 0 ? 0 :
                context.maxValue < 0 ? axisLength :

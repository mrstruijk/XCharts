...
namespace XCharts
{
        ...
        where T : Axis
    {
        ...
        protected virtual void UpdatePointerValue(Axis axis)
        {
            ...
            else
            {
                ...
                if (axis.IsCategory())
                {
                    ...
                    if (axis is YAxis)
                    {
                        var splitWid = AxisHelper.GetDataWidth(axis, grid.context.height, dataCount, dataZoom);

                        for (var j = 0; j < axis.GetDataCount(dataZoom); j++)
                        {
                            var pY = grid.context.y + j * splitWid;
                    ...
                    {
                        var splitWid = AxisHelper.GetDataWidth(axis, grid.context.width, dataCount, dataZoom);

                        for (var j = 0; j < axis.GetDataCount(dataZoom); j++)
                        {
                            var pX = grid.context.x + j * splitWid;
        ...
        internal virtual void UpdateAxisLabelText(Axis axis)
        {
            ...
            var runtimeWidth = axis is XAxis ? grid.context.width : grid.context.height;
        ...
        protected void InitAxis3D(Axis relativedAxis, Orient orient)
        {
            ...
            var textWidth = axis.axisLabel.width > 0 ? axis.axisLabel.width : AxisHelper.GetScaleWidth(axis, axisLength, 0, dataZoom);

            var textHeight = axis.axisLabel.height > 0 ? axis.axisLabel.height : 20f;

            var isPercentStack = SeriesHelper.IsPercentStack<Bar>(chart.series);
            var inside = axis.axisLabel.inside;

            var defaultAlignment = orient == Orient.Horizonal ? TextAnchor.MiddleCenter :
                (inside && axis.IsLeft()) || (!inside && axis.IsRight()) ? TextAnchor.MiddleLeft :
                                ...
                                float axisStartX, float axisStartY, float axisLength, float relativedLength)
        {
            ...
            var textWidth = axis.axisLabel.width > 0 ? axis.axisLabel.width :
                orient == Orient.Horizonal ? AxisHelper.GetScaleWidth(axis, axisLength, 0, dataZoom) :
                ...
                if (orient == Orient.Horizonal)
                {
                    ...
                    switch (axis.axisName.labelStyle.position)
                    {
                        case LabelStyle.Position.Start:
                            ...
                            label.SetActive(axis.axisName.labelStyle.show, true);
                            label.SetPosition(axis.position == Axis.AxisPosition.Top ? new Vector2(zeroPos.x - offset.x, axisStartY + relativedLength + offset.y + axis.offset) : new Vector2(zeroPos.x - offset.x, posY + offset.y));
                        ...
                        case LabelStyle.Position.Middle:
                            ...
                            label.SetActive(axis.axisName.labelStyle.show, true);
                            label.SetPosition(axis.position == Axis.AxisPosition.Top ? new Vector2(axisStartX + axisLength / 2 + offset.x, axisStartY + relativedLength - offset.y + axis.offset) : new Vector2(axisStartX + axisLength / 2 + offset.x, posY + offset.y));
                        ...
                        default:
                            ...
                            label.SetActive(axis.axisName.labelStyle.show, true);
                            label.SetPosition(axis.position == Axis.AxisPosition.Top ? new Vector2(axisStartX + axisLength + offset.x, axisStartY + relativedLength + offset.y + axis.offset) : new Vector2(axisStartX + axisLength + offset.x, posY + offset.y));
                ...
                else
                {
                    ...
                    switch (axis.axisName.labelStyle.position)
                    {
                        case LabelStyle.Position.Start:
                            ...
                            label.SetActive(axis.axisName.labelStyle.show, true);
                            label.SetPosition(axis.position == Axis.AxisPosition.Right ? new Vector2(axisStartX + relativedLength + offset.x + axis.offset, axisStartY - offset.y) : new Vector2(posX + offset.x, axisStartY - offset.y));
                        ...
                        case LabelStyle.Position.Middle:
                            ...
                            label.SetActive(axis.axisName.labelStyle.show, true);
                            label.SetPosition(axis.position == Axis.AxisPosition.Right ? new Vector2(axisStartX + relativedLength - offset.x + axis.offset, axisStartY + axisLength / 2 + offset.y) : new Vector2(posX + offset.x, axisStartY + axisLength / 2 + offset.y));
                        ...
                        default:
                            ...
                            label.SetActive(axis.axisName.labelStyle.show, true);
                            label.SetPosition(axis.position == Axis.AxisPosition.Right ? new Vector2(axisStartX + relativedLength + offset.x + axis.offset, axisStartY + axisLength + offset.y) : new Vector2(posX + offset.x, axisStartY + axisLength + offset.y));
            ...
            if (AxisHelper.NeedShowSplit(axis))
            {
                ...
                for (var i = 0; i < size; i++)
                {
                    ...
                    if (axis.axisTick.show)
                    {
                        if (orient == Orient.Horizonal)
                        {
                            var pX = axis.IsTime() ? startX + axis.GetDistance(axis.GetLabelValue(i), axisLength) : current;
                        ...
                        {
                            var pY = axis.IsTime() ? startY + axis.GetDistance(axis.GetLabelValue(i), axisLength) : current;

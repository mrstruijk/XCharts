    ...
    public class TooltipView
    {
        ...
        public void Refresh()
        {
            ...
            for (var i = 0; i < data.param.Count; i++)
            {
                ...
                for (var j = param.columns.Count; j < item.columns.Count; j++)
                {
                    item.columns[j].SetActive(false);
                }
            }

            for (var i = data.param.Count; i < m_Items.Count; i++)
            {
        ...
        public static TooltipView CreateView(Tooltip tooltip, ThemeStyle theme, Transform parent)
        {
            ...
            view.background.color = ChartHelper.IsClearColor(tooltip.backgroundColor) ? Color.white : tooltip.backgroundColor;

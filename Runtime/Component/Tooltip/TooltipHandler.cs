...
namespace XCharts.Runtime
{
    ...
    {
        private readonly Dictionary<string, ChartLabel> m_IndicatorLabels = new();
        ...
        private void UpdateTooltipIndicatorLabelText(Tooltip tooltip)
        {
            ...
            if (tooltip.context.type == Tooltip.Type.None)
            {
                return;
            }
        ...
        private void SetTooltipIndicatorLabel(Tooltip tooltip, Axis axis, ChartLabel label)
        {
            ...
            if (double.IsNaN(axis.context.pointerValue))
            {
                return;
            }
        ...
        private void UpdatePointerContainerAndSeriesAndTooltip(Tooltip tooltip, ref List<Serie> list)
        {
            ...
            for (var i = chart.components.Count - 1; i >= 0; i--)
            {
        ...
        private void GetSerieDataIndexByAxis(Serie serie, Axis axis, GridCoord grid, int dimension = 0)
        {
            ...
            if (!isTimeAxis)
            {
                ...
                serie.context.sortedData.Sort(delegate(SerieData a, SerieData b) { return a.GetData(dimension).CompareTo(b.GetData(dimension)); });
        ...
        private bool SetSerieTooltip(Tooltip tooltip, List<Serie> series)
        {
            ...
            for (var i = 0; i < series.Count; i++)
            {
                ...
                if (isTriggerByItem && serie.context.pointerItemDataIndex < 0)
                {
                    continue;
                }
        ...
        private bool IsYCategoryOfGrid(int gridIndex)
        {
            ...
            foreach (var component in chart.GetChartComponents<XAxis>())
            {
                ...
                if (xAxis.gridIndex == gridIndex && xAxis.IsCategory())
                {
                    return false;
                }
            }
        ...
        private void DrawXAxisIndicator(VertexHelper vh, Tooltip tooltip, GridCoord grid)
        {
            ...
            foreach (var component in xAxes)
            {
                ...
                if (xAxis.gridIndex == grid.index)
                {
                    ...
                    var splitWidth = AxisHelper.GetDataWidth(xAxis, grid.context.width, dataCount, dataZoom);

                    switch (tooltip.context.type)
                    {
                        case Tooltip.Type.Cross:
                        case Tooltip.Type.Line:
                            var pX = grid.context.x;
                            pX += xAxis.IsCategory() ? (float) (xAxis.context.pointerValue * splitWidth + (xAxis.boundaryGap ? splitWidth / 2 : 0)) : xAxis.GetDistance(xAxis.context.axisTooltipValue, grid.context.width);
                            ...
                            var sp = new Vector2(pX, grid.context.y);
                            var ep = new Vector2(pX, grid.context.y + grid.context.height);
                        ...
                        case Tooltip.Type.Shadow:
                            ...
                            {
                                var tooltipSplitWid = splitWidth < 1 ? 1 : splitWidth;
                                ...
                                var pY = grid.context.y + grid.context.height;
                                var p1 = chart.ClampInGrid(grid, new Vector3(pX, grid.context.y));
                                var p2 = chart.ClampInGrid(grid, new Vector3(pX, pY));
                                var p3 = chart.ClampInGrid(grid, new Vector3(pX + tooltipSplitWid, pY));
                                var p4 = chart.ClampInGrid(grid, new Vector3(pX + tooltipSplitWid, grid.context.y));
        ...
        private void DrawYAxisIndicator(VertexHelper vh, Tooltip tooltip, GridCoord grid)
        {
            ...
            foreach (var component in yAxes)
            {
                ...
                if (yAxis.gridIndex == grid.index)
                {
                    ...
                    var dataCount = chart.series.Count > 0 ? chart.series[0].GetDataList(dataZoom).Count : 0;
                    var splitWidth = AxisHelper.GetDataWidth(yAxis, grid.context.height, dataCount, dataZoom);

                    switch (tooltip.context.type)
                    {
                        case Tooltip.Type.Cross:
                        case Tooltip.Type.Line:
                            var pY = (float) (grid.context.y + yAxis.context.pointerValue * splitWidth +
                            ...
                            var sp = new Vector2(grid.context.x, pY);
                            var ep = new Vector2(grid.context.x + grid.context.width, pY);
                            var lineColor = TooltipHelper.GetLineColor(tooltip, chart.theme.tooltip.lineColor);
                            ChartDrawer.DrawLineStyle(vh, lineType, lineWidth, sp, ep, lineColor);
                        ...
                        case Tooltip.Type.Shadow:
                            ...
                            {
                                var tooltipSplitWid = splitWidth < 1 ? 1 : splitWidth;
                                var pX = grid.context.x + grid.context.width;
                                ...
                                var p1 = new Vector3(grid.context.x, pY);
                                var p2 = new Vector3(grid.context.x, pY + tooltipSplitWid);
                                var p3 = new Vector3(pX, pY + tooltipSplitWid);
                                var p4 = new Vector3(pX, pY);

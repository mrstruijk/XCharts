...
namespace XCharts.Runtime
{
    ...
    public class VisualMapRange : ChildComponent
    {
        ...
        public bool Contains(double value, double minMaxRange)
        {
            ...
            var cmin = Math.Abs(m_Min) < 1 ? minMaxRange * m_Min : m_Min;
            var cmax = Math.Abs(m_Max) < 1 ? minMaxRange * m_Max : m_Max;
    ...
    public class VisualMap : MainComponent
    {
        ...
        [SerializeField] private List<VisualMapRange> m_OutOfRange = new() {new VisualMapRange() {color = Color.gray}};
        [SerializeField] private List<VisualMapRange> m_InRange = new();

        public VisualMapContext context = new();
        ...
        /// <summary>
        ///     The minimum allowed. 'min' must be user specified. [visualmap.min, visualmap.max] forms the "domain" of the
        ///     visualMap.
        ///     ||
        ...
        /// <summary>
        ///     The maximum allowed. 'max' must be user specified. [visualmap.min, visualmap.max] forms the "domain" of the
        ///     visualMap.
        ///     ||
        ...
        public double max
        {
            ...
            set
            {
                m_Max = value < min ? min + 1 : value;
                SetVerticesDirty();
            }
        }

        /// <summary>
        ///     Specifies the position of the numeric value corresponding to the handle. Range should be within the range of
        ///     [min,max].
        ///     ||
        ...
        public override bool vertsDirty => m_VertsDirty || location.anyDirty;
        ...
        public float runtimeRangeMinHeight => (float) ((rangeMin - min) / (max - min) * itemHeight);
        public float runtimeRangeMaxHeight => (float) ((rangeMax - min) / (max - min) * itemHeight);
        ...
        public Color32 GetColor(double value)
        {
            ...
            var splitNumber = m_InRange.Count;
        ...
        private bool IsNeedPieceColor(double value, out int index)
        {
            ...
            for (var i = 0; i < m_InRange.Count; i++)
            {
